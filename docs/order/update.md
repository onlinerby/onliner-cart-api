...
Для новых заказов доступна возможность уменьшения стоиомости доставки заказа, до момента передачи в доставку (пока заказ находится в статусах `processing`, `confirmed`).

В момент передачи заказа в доставку для новых заказов можно указать комментарий к доставке, который увидит покупатель.

---

Для старых заказов доступно только изменение статуса заказа.

Таблица допустимых переходов статусов для старых заказов для магазина:
