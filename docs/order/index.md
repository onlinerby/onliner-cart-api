...
Блоки данных `converted` с информацией о ценах в BYN и BYR используются до 31.12.2016 (в течении переходного периода на время деноменации) и могут быть удалены без предупреждения.
...
|Параметр|Ошибки|
|---|---|
|page|Значение поля должно быть целым числом, Значение поля должно быть не менее 1|
|limit|Значение поля должно быть целым числом, Значение поля должно быть не менее 1, Значение поля не может быть больше 100|
|status|Значение поля должно быть строкой, Недопустимый статус|
